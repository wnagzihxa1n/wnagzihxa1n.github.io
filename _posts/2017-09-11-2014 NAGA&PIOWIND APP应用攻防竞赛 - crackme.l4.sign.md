...
![IMAGE](/assets/resources/729F23F05581B500ECBA2FF1F882D514.png)
...
![IMAGE](/assets/resources/F11E1E8409E1E2B5E2D39F2A08CCF319.png)

但是我们跟入后，发现壳好像没有脱干净（后来发现其实不是没脱干净）

![IMAGE](/assets/resources/9B59DB5BBEC0C80300968F7E6C5598F2.png)

再次动态调试脱壳，这次我们找到校验函数，单步跟下去看看具体是什么情况

我们需要先找到校验函数的地址，使用给`dvmUseJNIBridge`函数下断点的方法

![IMAGE](/assets/resources/AE96DAA7C380FAFFD47956B551A672CB.png)
...
![IMAGE](/assets/resources/C6BF6C879306ADADAF77C2B1A2D99F75.png)

跟过去，可以看到确实是校验函数

![IMAGE](/assets/resources/E088048C6478510C7DBDA7556973196E.png)

找到我们看到是跳转地址的地方

![IMAGE](/assets/resources/BA2BC211D65FE53F559B390A8F4AB32A.png)

双击过去

![IMAGE](/assets/resources/E5AB4F7BB8FF0252528356596200CC3A.png)

再次双击过去，发现是关键加解密点了，此时我们记录一下这个地址

![IMAGE](/assets/resources/3FA0182980B36B78B72DF09754CB85E1.png)

再次dump这个so文件

动静结合，接下来看能力了

![IMAGE](/assets/resources/0EC7B8CD2BB1CE8F2A910F4BF0268E81.png)

入口的数据初始化，然后调用`_Unwind_GetCFAB`，这和前几题是类似的

![IMAGE](/assets/resources/E7164308A313E2F607131E81260C065A.png)
...
![IMAGE](/assets/resources/125343C2DA359FAAFF16938734BC20EC.png)
...
![IMAGE](/assets/resources/5BE7D64D585CA33537ACA67889C11459.png)
...
![IMAGE](/assets/resources/D966598681C23575C2DF39702233DC05.png)

这是整个大循环

![IMAGE](/assets/resources/B1D0E4343F1F751F0E474D8224AC6132.png)
...
![IMAGE](/assets/resources/D082E9F4D6609D960699AB8B2D298A56.png)

接下来是校验的地方，单步走一遍先，找到关键的地方，可以看到这里调用了四个函数

![IMAGE](/assets/resources/F2384491B4C045081BA4B379975EF797.png)

但是在静态时这位置我是手动找的，这个费劲，有的函数没有识别出来，红色的。。。。。。

![IMAGE](/assets/resources/20B52228AF91A264ABC5C79811896269.png)
...
![IMAGE](/assets/resources/F2B6BC6CC15BC4304C407341A1B6B73A.png)
...
![IMAGE](/assets/resources/12CCEFC73C98AFAD3E4A3445EBF5B9B6.png)
...
![IMAGE](/assets/resources/D26D0D44367408B05050FFC1C1647D66.png)
...
![IMAGE](/assets/resources/05D23A9EF243BC42F029D7D2D6453CC7.png)
...
![IMAGE](/assets/resources/F67FDA39F1DCE4EE13C67E33E44ED964.png)

在上图的位置下个断点，数据区跟随`R3`，可以看到完整的生成过程

第三个函数，就一个小循环，应该比较简单

![IMAGE](/assets/resources/889FCF8F529600C21E23D3863CD08B1D.png)
...
![IMAGE](/assets/resources/850A6D1E53CAED79A11F4F495067F5AF.png)
...
![IMAGE](/assets/resources/21057AE145D06E84F11B1982080539F2.png)

但是在静态分析的时候是空的，这里有一个`0x30`的偏移

![IMAGE](/assets/resources/D8E4092BC4661D650501B274AC4DFDBC.png)
...
![IMAGE](/assets/resources/9438CCA14475E46D51CF53F3A53757BA.png)
...
![IMAGE](/assets/resources/B75C768FA33559A407435FE2A3793FAE.png)
...
![IMAGE](/assets/resources/49E0D72A120B15DED0C896B9F3BB4C0D.png)
...
![IMAGE](/assets/resources/B32717F89A35230554B695DC0EC76349.png)

然后两组计算完后，得到解密后的数据

![IMAGE](/assets/resources/811CE4E8B97A5446519AFAA963AB8751.png)

那这个就很尴尬了，怎么会多出八位

百撕不得姐，于是找老司机求教

![IMAGE](/assets/resources/1EC4CB3F4089E4018CF6735A482F99A2.png)

发现用Java的加解密库计算出来的数据并不正确，其实可能是校验的过程改了

正常情况下解密出来的数据应该是这样的

![IMAGE](/assets/resources/E58C84ABEBE883CB4A1432785E912BDD.png)
...
![IMAGE](/assets/resources/CBC29C4253BEBDAFA3EBFD45434B091F.png)
...
![IMAGE](/assets/resources/EADBB5EBC3523E90CF43CE0D98BAC533.png)
...
![IMAGE](/assets/resources/21ED8DB3AA296F44D02532808AA998C2.png)
...
![IMAGE](/assets/resources/3E7CDB99EE3FB5BB690CBA45A1306120.png)
